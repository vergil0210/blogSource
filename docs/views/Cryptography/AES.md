...
**进阶加密标准**（英语：**A**dvanced **E**ncryption **S**tandard，[缩写](https://zh.wikipedia.org/wiki/缩写)：AES），是[美国联邦政府](https://zh.wikipedia.org/wiki/美国联邦政府)采用的一种[区块加密](https://zh.wikipedia.org/wiki/區塊加密)标准。这个标准用来替代原先的[DES](https://zh.wikipedia.org/wiki/DES)，已经被多方分析且广为全世界所使用。经过五年的甄选流程，进阶加密标准由[美国国家标准与技术研究院](https://zh.wikipedia.org/wiki/美国国家标准与技术研究院)（NIST）于2001年11月26日发布于FIPS PUB 197，并在2002年5月26日成为有效的标准。现在，进阶加密标准已然成为[对称密钥加密](https://zh.wikipedia.org/wiki/对称密钥加密)中最流行的[演算法](https://zh.wikipedia.org/wiki/演算法)之一。
...
**注意**：加密算法输入的128字节明文是按照前四个字节放入第一列，接着四个字节放在第二列..这样的顺序放入的
...
如上所说，AES的加解密算法不同，尽管**加解密的密钥扩展是一样的，但加、解密中变换的顺序是不同**。这就导致对于同时需要加解密的应用而言，需要两个不同的软件版本。**等价逆算法与加密算法有同样的结构，他的变换顺序与加密是相同的**(通过逆变换)。

通过如下的方式改进解密算法，使其与加密算法的结构一致：

- 交换逆向行移位和逆向字节代替

  逆向行移位只影响状态中字节的顺序，不更改字节内容。

  逆向字节代替更改字节内容，但不影响字节顺序，也不依赖字节顺序。

  故：**逆向行移位[逆向字节代替(S<sub>i</sub>)] = 逆向字节代替[逆向行移位(S<sub>i</sub>)] **二者替换不影响整体结果

- 交换逆向轮密钥加和逆向列混淆

  轮密钥加和逆向列混淆并不状态数组中的字节顺序

  即：逆向列混淆(S<sub>i</sub>+w<sub>i</sub>) = [逆向列混淆(S<sub>i</sub>)]+[逆向列混淆(w<sub>i</sub>)]

替换后效果如下图所示：

![图示](../img/2021071501.png)
...
字节代替作用对象是 16bytes 的状态数组，每个字节各自通过S盒映射成一个新的字节。

字代替的对象是密钥扩展算法中 4bytes 的输入字，每个字节各自通过S盒映射成一个新的字节。

### 3.4 行移位和字循环函数有何不同

行移位的作用对象是 16bytes 的状态数组，第一行不变，第二行循环左移一位，第三行循环左移两位，第四行循环左移三位。

字循环的作用对象是 4bytes 的输入字，使其循环左移一位。

### 3.5 AES解密算法何其他等价逆算法之间有何不同

对于AES解密算法，尽管加解密的密钥扩展是一样的，但加、解密中变换的顺序是不同的。

而等价逆算法的变换顺序与加密是相同的(通过逆变换)
